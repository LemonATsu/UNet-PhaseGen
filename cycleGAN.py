...
class AEModel(nn.Module):
    def __init__(self, input_nc, output_nc, n_f=3, n_t=2, batch_size=1, hidden=256, gpu_ids=[], norm_layer=nn.BatchNorm2d, trp=False):
        ...
        encoder = [
            ...
            nn.Conv1d(input_nc, input_nc * 2, kernel_size=16, stride=1, padding=2), # 1024, 100
            norm_layer(input_nc * 2),
            nn.LeakyReLU(0.02, True),
            nn.Conv1d(input_nc * 2, input_nc * 2, kernel_size=16, stride=2, padding=1), # 1024, 44
            norm_layer(input_nc * 2),
            nn.LeakyReLU(0.02, True),
            nn.Conv1d(input_nc * 2, input_nc * 4, kernel_size=4, stride=2, padding=1), # 1024, 22
            norm_layer(input_nc * 4),
            nn.LeakyReLU(0.02, True),
            View(batch_size, -1),
            nn.Linear(input_nc * 22 * 4, 512) if not trp else nn.Linear(input_nc // 4 * 64, 256)
        ]

        decoder = [
            nn.Linear(512, input_nc  * 22 * 4) if not trp else nn.Linear(256, input_nc // 4 * 64),
            View(batch_size, input_nc * 4, 22) if not trp else View(batch_size, input_nc // 4, 64),
            # decode time
            nn.ConvTranspose1d(input_nc * 4, input_nc * 2, kernel_size=4, stride=2, padding=1), # 1024, 16
            norm_layer(input_nc * 2),
            nn.ReLU(True),
            nn.ConvTranspose1d(input_nc * 2, input_nc * 2, kernel_size=16, stride=2, padding=1), # 512, 32
            norm_layer(input_nc * 2),
            nn.ReLU(True),
            nn.ConvTranspose1d(input_nc * 2, input_nc, kernel_size=16, stride=1, padding=2), # 1024, 64
            norm_layer(input_nc),
            nn.ReLU(True),
            nn.ConvTranspose1d(input_nc, input_nc, kernel_size=32, stride=1, padding=7),
            norm_layer(input_nc),
            nn.ReLU(True),
